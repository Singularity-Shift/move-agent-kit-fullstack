...
export class PanoraPriceTool extends Tool {
	name = "panora_aggregator_price"
	description = `this tool can be used to get token price in panora - liquidity aggregator on aptos

  if you want to get the price of APT the token address needs to be "0x1::aptos_coin::AptosCoin"
  If user prompt doesn't specify if USDT is layerzero, the token address needs to be "0x357b0b74bc833e95a115ad22604854d6b0fca151cecd94111770e5d6ffc9dc2b"
  If user prompt doesn't specify if USDC is layerzero, the token address needs to be "0xbae207659db88bea0cbead6da0ed00aac12edcdda169e591cd41c94180b46f3b"
  Please keep message short and clear for better understanding.
  
  Popular tokens which are include to get price:
  ...
  Examples:
  - "I want to get the price of CASH"
  - "Show me the Price of USDC"
  - "What is the price of EDOG?"

  Inputs ( input is a JSON string ):
  tokeAddress: string, eg "0x1::aptos_coin::AptosCoin" or "0xf22bede237a07e121b56d91a491eb7bcdfd1f5907926a9e58338f964a01b17fa::asset::USDT" (required)
  `
	...
	protected async _call(input: string): Promise<string> {
		try {
			...
			const mintDetail = await this.agent.getTokenDetails(parsedInput.tokenAddress)

			const priceData = await this.agent.priceWithPanora(parsedInput.tokenAddress)
